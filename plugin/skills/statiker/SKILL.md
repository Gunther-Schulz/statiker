...
- a basis citing ANOTHER record's entries names the record (tracker
  ...
  bare code pointer (`tools/x.py:40`) or an ordinary `label:`, which
  name nothing readable as a record and leave the id resolving in
  THIS run's namespace as usual; the exemption PERSISTS across a
...
live closure. The red-first pin the unit's design named is
