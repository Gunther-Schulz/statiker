...
# Two (tracker, golden) pairs: the rich tracker carries 28 codes and
# cannot ALSO carry admission-window (a single header's Status/Phase
# fields are each either invalid — status-enum/phase-enum's own rows
# — or valid-and-past-ADMISSION_WINDOW, never both on one field in
# one header); tracker-admission.md is the "different single-tracker
# file" the rich tracker's own EXEMPT reason names — a minimal
# fixture whose Phase line is valid and placed past line 20, carrying
# admission-window ALONE. Together the two pairs reach all 29
# RULE_MINT_VERSION codes and EXEMPT is empty.
GOLDEN_PAIRS = (
    (GOLDEN_DIR / "tracker.md", GOLDEN_DIR / "expected-violations.json"),
    (GOLDEN_DIR / "tracker-admission.md",
     GOLDEN_DIR / "expected-violations-admission.json"),
)


class TestGoldenCorpusSweep(unittest.TestCase):
    """st-28: a regression battery over `sweep`'s full hit SET on two
    rich, COMMITTED trackers (GOLDEN_PAIRS) — the class of defect the
    0.2.84 checkpoint review's B1/B2 slipped through a 482-green
    suite: `basis_id_citations` silently NARROWED across versions (a
    live-basis check went green on citation shapes it correctly fired
    on at c19c829) because the battery's own tests were derived from
    the new helper's intent — same-parentage expectations move with
    the mutant, so nothing caught the regression
    (dev-notes/OBSERVATIONS.md, 2026-09-10 checkpoint-review
    dispositions, B1/B2). The rich tracker's incident rows (F20-F25,
    its Block: basis-cites-invalidated section) are the exact B1/B2
    citation shapes: `(F20)`, `F20)`, `F20.`, `tools/x.py:40 F20`,
    `the probe: F20`.
    ...
    age loudly). Red on ANY code's hits shrinking or growing in
    EITHER pair (test_sweep_hit_set_matches_golden, one subTest per
    pair), and on a new code minted with no corpus row anywhere
    across both goldens (test_coverage_matches_rule_mint_version).

    SECOND PAIR, provenance (dispatcher follow-up after the st-28
    commit, same brief): the first cut's EXEMPT={admission-window}
    read the brief's own admission rule ("a code sweep cannot emit
    over A single tracker file") too literally — sweep DOES emit
    admission-window over a DIFFERENT single tracker (the reason
    string said so), so as built the code could silently stop firing
    with no red anywhere. tracker-admission.md is that different
    ...
    re-run by the suite itself, since these arms need historical tool
    checkouts or mutate the working tree):
      First pair, three arms (st-28 original commit):
      1. INCIDENT (shrink) — `sweep` over tracker.md with the tool
         ...
         changes how tracker.md's declarator-bookkeeping block (lines
         31-33) parses under both old tool versions — noted, not a
         gap, since the required incident rows (F21-F25) behaved
         exactly as predicted.
      2. GROWTH — dropping hold_violations' AUTO-ACCEPTED exemption
         in the working tree makes hold-form ALSO fire on tracker.md's
         clean control (F19, line 28) — an ADDED hit vs golden;
         reverted (`git checkout --`), `git diff` empty after.
      3. REACH — adding `"golden-probe-code": "0.0.0"` to
         ...
         (the new code has no corpus row in either golden); reverted
         the same way.
      Second pair, two arms (this follow-up):
      4. COVERAGE-FIRST — with EXEMPT emptied and only the first pair
         registered, test_coverage_matches_rule_mint_version FAILS,
         naming admission-window as missing; adding the second pair
         makes it PASS.
      5. SUPPRESSION — a targeted string replace in the working tree
         (never a whole-file `git checkout`, since other uncommitted
         work can share the file) makes the admission-window `elif`
         branch for Phase a no-op; running sweep over
         tracker-admission.md then shows admission-window REMOVED
         from the produced hit set — the pair's own
         test_sweep_hit_set_matches_golden subTest fails; the reverse
         replace restores it, `git diff -- plugin` empty after."""
    ...
    def _sweep_verdict(self, tracker):
        r = subprocess.run(
            [sys.executable, str(SCRIPT), "sweep", "--tracker", str(tracker)],
    ...
    def _hit_set(self, verdict, tracker_lines):
        out = set()
        for v in verdict.get("violations", []):
            ln = v["line"]
            text = (tracker_lines[ln - 1].strip()
                    if 0 < ln <= len(tracker_lines) else "")
            out.add((ln, v["code"], text))
        return out

    def test_sweep_hit_set_matches_golden(self):
        for tracker, golden_path in GOLDEN_PAIRS:
            with self.subTest(tracker=tracker.name):
                ...
                if os.environ.get("STATIKER_GOLDEN_REGEN") == "1":
                    ...
                    golden_path.write_text(
                        json.dumps(golden_list, indent=2,
                                  ensure_ascii=False) + "\n")
                    self.fail(
                        f"golden regenerated — review `git diff "
                        f"{golden_path.relative_to(REPO_ROOT)}` before "
                        f"committing")
                ...
                if removed or added:
                    msg = [f"golden-corpus sweep mismatch on {tracker.name} "
                          f"(parsed sets, not rendered text):"]
                    for l, c, t in sorted(removed):
                        msg.append(f"REMOVED (in golden, not produced): "
                                  f"{c} @ line {l}: {t}")
