# Raw evidence — poteto, "The Complete Guide to pstack Pt. 1: Verification is all you need"

Provenance: article by Lauren Tan (@poteto), published and updated
2026-08-31, at x.com/poteto/article/2094457600259842065. Supplied by the
operator 2026-09-13 as pasted text (the site's own rendering; the code
blocks below are absent from a plain fetch of the URL, which is why this
copy exists). Trailing site chrome dropped; body kept verbatim.

Quoted material — exempt from the English-only rule as source text, and
NOT to be corrected in place: quotes cited elsewhere must match these
bytes.

Consumer: `dev-notes/pstack-comparison-2026-09-13.md` §7a (the apparatus
asymmetry), items st-47 (the series), st-48 (the FP5 apparatus scope
question), st-49 (the lever clause). The CLI verb list and the feature-map
entry format below are the concrete spec any st-48 design would work from.

Note on date: 2026-08-31 is TEN DAYS BEFORE the 2026-09-10 pstack
comparison. This was available and unread then.
