# Raw evidence — poteto, "The Complete Guide to pstack Pt. 2"

Provenance: article by Lauren Tan (@poteto) at
x.com/poteto/article/2097732320606507506, posted ~2026-09-10 (search
reported "3 days ago" on 2026-09-13). Fetched 2026-09-13 by
`mcp__fetch__fetch`; that fetch strips code blocks and images, so the
prompt examples below appear as the fetch rendered them — where a
verbatim code block matters, re-fetch the rendered page. Part 3 is NOT
published as of this date; the article closes "stay tuned for Part 3!".

Quoted material — exempt from the English-only rule as source text, and
NOT to be corrected in place.

Consumer: item st-47 (the series); `dev-notes/pstack-comparison-2026-09-13.md`
§2b and §9 (the design-grader zero, which this article converts from an
omission into a stated refusal); the PLAN medium-tenet disambiguation
(2026-08-08 entry), which this article partly refutes.

LOAD-BEARING FOR US, flagged before the body so a later reader does not
have to find it: (1) "It's also why I never bother with reviewing
abstract plans adversarially" — an explicit, argued rejection of
statiker's forcing point 3, not an omission. (2) "The truth is that I do
plan, but I do so through code" — she DOES design; the medium is code.
(3) `/architect`'s "Scrap when the design is wrong" plus the empirical
falsifier (repeated workarounds, `any` casts) — design falsification
exists in her system, but it is EMPIRICAL and DOWNSTREAM of
implementation rather than adversarial and upstream.
